...
public class UserDAO {
	...
	public User get(final int id) {
		try (final Connection connection = ConnectionHelper.getConnection()) {
			final PreparedStatement statement = connection.prepareStatement("select user_id, name from users where user_id = ?");
	...
	public User update(final int id, final UserDTO dto) {
		try (final Connection connection = ConnectionHelper.getConnection()) {
			...
			final PreparedStatement statement = connection.prepareStatement("update users set name = ? where user_id = ?");
			statement.setString(1, dto.getName());
